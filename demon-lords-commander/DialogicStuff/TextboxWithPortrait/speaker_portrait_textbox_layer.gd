...
func _apply_responsive_layout() -> void:
	...
	var panel_w: float = vp.x * 0.70
	var panel_h: float = vp.y * 0.325
	var margin_bottom: float = vp.y * 0.15
	panel.offset_left = -panel_w * 0.5
	panel.offset_right = panel_w * 0.5
	panel.offset_top = -panel_h - margin_bottom
	...
	var font_size: int = int(vp.y * 0.033)
	var name_size: int = int(vp.y * 0.045)
...
func _apply_export_overrides() -> void:
	...
	var stylebox: StyleBoxFlat = (load(box_panel) as StyleBoxFlat).duplicate()
	stylebox.skew = Vector2.ZERO
